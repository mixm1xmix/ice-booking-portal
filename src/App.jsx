...
function generateSlots() {
  ...
  return times.map(([h, m]) => {
    ...
    return { id: `${h}-${m}`, label: `${fmt(h,m)} – ${fmt(endH,endM)}`, cleaning: CLEANING_SLOTS.has(`${h}-${m}`) };
  });
}
...
// Colours
const RED="#cc0001",RED_DARK="#990001",RED_LIGHT="#fde8e8",RED_BORDER="#e8a0a0";
const CHARCOAL="#1e1e1e",MID_GREY="#555555",LIGHT_GREY="#f5f5f5",BORDER_GREY="#dedede";
const WHITE="#ffffff",GREEN="#1a7a3c",GREEN_BG="#e8f5ee",GREEN_BORDER="#7ec89a";
const ICE_BLUE="#e8f4fb",ICE_BORDER="#aed6ef";
const AMBER="#7a4f00",AMBER_BG="#fff8e6",AMBER_BORDER="#f0c040";
const NAVY="#0a2a4a",NAVY_LIGHT="#e8eef5";
...
export default function CurlingBooking() {
  ...
  const [adminConfig, setAdminConfig] = useState({ masterPin: DEFAULT_MASTER_PIN, secondaryPins: [] });
  const [adminPinInput, setAdminPinInput] = useState("");
  const [adminAuthed, setAdminAuthed] = useState(null); // null | "master" | "secondary"
  const [adminPinError, setAdminPinError] = useState("");
  const [adminActiveClub, setAdminActiveClub] = useState(0);

  // Admin config editing
  const [newMasterPin, setNewMasterPin] = useState("");
  const [newSecPin, setNewSecPin] = useState("");
  const [pinMsg, setPinMsg] = useState(null);

  // Manual assign modal
  const [assignModal, setAssignModal] = useState(null); // { club, slotId, slotLabel, sheet } | null
  ...
  async function loadBookings() {
    try {
      ...
      if (r) setBookings(JSON.parse(r.value));
    } catch(e) {}
    setLoading(false);
  }
  ...
  async function loadAdminConfig() {
    try {
      const r = await window.storage.get(ADMIN_STORAGE_KEY, true);
      if (r) setAdminConfig(JSON.parse(r.value));
    } catch(e) {}
  }

  async function saveBookings(updated) {
    try { await window.storage.set(STORAGE_KEY, JSON.stringify(updated), true); } catch(e) {}
  }

  async function saveAdminConfig(cfg) {
    try { await window.storage.set(ADMIN_STORAGE_KEY, JSON.stringify(cfg), true); setAdminConfig(cfg); } catch(e) {}
  }
  ...
  async function handleBook(club, slot) {
    ...
    let fresh = {};
    try { const r = await window.storage.get(STORAGE_KEY, true); if (r) fresh = JSON.parse(r.value); } catch(e) {}
    ...
    let sheet = 1; while (takenSheets.includes(sheet)) sheet++;
  ...
  async function handleCancel(key) {
    ...
    const updated = { ...bookings }; delete updated[key];
    await saveBookings(updated); setBookings(updated);
  ...
  function handleAdminLogin() {
    ...
    if (pin === adminConfig.masterPin) { setAdminAuthed("master"); setAdminPinError(""); setAdminPinInput(""); }
    else if (adminConfig.secondaryPins.includes(pin)) { setAdminAuthed("secondary"); setAdminPinError(""); setAdminPinInput(""); }
    else { setAdminPinError("Incorrect PIN. Please try again."); }
  }

  function handleChangeMasterPin() {
    if (adminAuthed !== "master") return;
    const p = newMasterPin.trim();
    if (!/^\d{4,8}$/.test(p)) { showPinMsg("Master PIN must be 4–8 digits."); return; }
    const cfg = { ...adminConfig, masterPin: p };
    saveAdminConfig(cfg);
    setNewMasterPin("");
    showPinMsg("Master PIN updated.", "success");
  }

  function handleAddSecPin() {
    if (adminAuthed !== "master") return;
    const p = newSecPin.trim();
    if (!/^\d{4,8}$/.test(p)) { showPinMsg("Secondary PIN must be 4–8 digits."); return; }
    if (p === adminConfig.masterPin) { showPinMsg("Secondary PIN cannot match the master PIN."); return; }
    if (adminConfig.secondaryPins.includes(p)) { showPinMsg("That PIN already exists."); return; }
    const cfg = { ...adminConfig, secondaryPins: [...adminConfig.secondaryPins, p] };
    saveAdminConfig(cfg);
    setNewSecPin("");
    showPinMsg(`Secondary PIN ${p} added.`, "success");
  }

  function handleRemoveSecPin(p) {
    const cfg = { ...adminConfig, secondaryPins: adminConfig.secondaryPins.filter(x => x !== p) };
    saveAdminConfig(cfg);
  }
  ...
  if (view === "admin") {
    return (
      <div style={{ minHeight:"100vh", background:LIGHT_GREY, fontFamily:"'Arial',sans-serif", color:CHARCOAL }}>
        {/* Header */}
        <div style={{ background:`linear-gradient(135deg,${NAVY} 0%,#1a4a7a 100%)`, boxShadow:"0 3px 12px rgba(0,0,0,0.3)" }}>
          ...
          <div style={{ padding:"16px 28px", display:"flex", alignItems:"center", gap:16, flexWrap:"wrap" }}>
            ...
            <div style={{ marginLeft:"auto", display:"flex", gap:10 }}>
              {adminAuthed && <span style={{ background:"rgba(255,255,255,0.15)", border:"1px solid rgba(255,255,255,0.3)", borderRadius:6, padding:"6px 14px", fontSize:13, color:WHITE, fontWeight:600 }}>{adminAuthed === "master" ? "🔑 Master Admin" : "🔐 Secondary Admin"}</span>}
              <button onClick={() => { setView("booking"); setAdminAuthed(null); setAdminPinInput(""); }} style={{ padding:"8px 18px", borderRadius:6, border:"1px solid rgba(255,255,255,0.4)", background:"rgba(255,255,255,0.1)", color:WHITE, fontWeight:600, cursor:"pointer", fontSize:14, fontFamily:"inherit" }}>← Back to Booking</button>
            </div>
          </div>
        </div>
          ...
          {!adminAuthed ? (
            <div style={{ background:WHITE, border:`1px solid ${BORDER_GREY}`, borderRadius:10, padding:"30px", maxWidth:420, margin:"40px auto", boxShadow:"0 2px 12px rgba(0,0,0,0.08)" }}>
              ...
              <input
                ...
                style={{ width:"100%", padding:"12px 14px", borderRadius:7, border:`2px solid ${adminPinError ? RED : BORDER_GREY}`, fontSize:22, letterSpacing:"0.3em", fontFamily:"inherit", color:CHARCOAL, boxSizing:"border-box", outline:"none" }}
              />
              {adminPinError && <div style={{ color:RED_DARK, fontSize:14, marginTop:8, fontWeight:600 }}>{adminPinError}</div>}
              <button onClick={handleAdminLogin} style={{ width:"100%", marginTop:16, padding:"13px 0", borderRadius:7, border:"none", background:NAVY, color:WHITE, fontWeight:700, fontSize:17, cursor:"pointer", fontFamily:"inherit" }}>Login</button>
            </div>
          ) : (
            <>
              {/* Message */}
              ...
              {adminAuthed === "master" && (
                <div style={{ background:WHITE, border:`1px solid ${BORDER_GREY}`, borderLeft:`5px solid ${NAVY}`, borderRadius:8, padding:"20px 22px", marginBottom:20, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
                  <h2 style={{ margin:"0 0 16px", fontSize:18, color:NAVY }}>🔑 PIN Management</h2>

                  {pinMsg && <div style={{ padding:"10px 14px", borderRadius:6, marginBottom:14, background:pinMsg.type==="success"?GREEN_BG:RED_LIGHT, border:`1px solid ${pinMsg.type==="success"?GREEN_BORDER:RED_BORDER}`, color:pinMsg.type==="success"?GREEN:RED_DARK, fontSize:14, fontWeight:600 }}>{pinMsg.text}</div>}

                  <div style={{ display:"flex", gap:20, flexWrap:"wrap" }}>
                    {/* Change master PIN */}
                    <div style={{ flex:"1 1 220px" }}>
                      <label style={{ fontSize:14, fontWeight:700, color:CHARCOAL, display:"block", marginBottom:7 }}>Change Master PIN</label>
                      <div style={{ display:"flex", gap:8 }}>
                        <input type="password" value={newMasterPin} onChange={e=>setNewMasterPin(e.target.value)} placeholder="New PIN (4–8 digits)" style={{ flex:1, padding:"9px 12px", borderRadius:6, border:`1px solid ${BORDER_GREY}`, fontSize:16, fontFamily:"inherit", outline:"none" }} />
                        <button onClick={handleChangeMasterPin} style={{ padding:"9px 16px", borderRadius:6, border:"none", background:NAVY, color:WHITE, fontWeight:700, cursor:"pointer", fontSize:14, fontFamily:"inherit" }}>Update</button>
                      </div>
                    </div>

                    {/* Add secondary PIN */}
                    <div style={{ flex:"1 1 220px" }}>
                      <label style={{ fontSize:14, fontWeight:700, color:CHARCOAL, display:"block", marginBottom:7 }}>Add Secondary Admin PIN</label>
                      <div style={{ display:"flex", gap:8 }}>
                        <input type="password" value={newSecPin} onChange={e=>setNewSecPin(e.target.value)} placeholder="New PIN (4–8 digits)" style={{ flex:1, padding:"9px 12px", borderRadius:6, border:`1px solid ${BORDER_GREY}`, fontSize:16, fontFamily:"inherit", outline:"none" }} />
                        <button onClick={handleAddSecPin} style={{ padding:"9px 16px", borderRadius:6, border:"none", background:GREEN, color:WHITE, fontWeight:700, cursor:"pointer", fontSize:14, fontFamily:"inherit" }}>Add</button>
                      </div>
                    </div>
                  </div>

                  {/* Secondary PINs list */}
                  {adminConfig.secondaryPins.length > 0 && (
                    <div style={{ marginTop:16 }}>
                      <div style={{ fontSize:14, fontWeight:700, color:CHARCOAL, marginBottom:8 }}>Active Secondary PINs</div>
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        {adminConfig.secondaryPins.map(p => (
                          <div key={p} style={{ display:"flex", alignItems:"center", gap:8, background:NAVY_LIGHT, border:`1px solid #c0d0e0`, borderRadius:6, padding:"6px 12px" }}>
                            <span style={{ fontSize:15, fontWeight:700, letterSpacing:"0.15em", color:NAVY }}>{"•".repeat(p.length)}</span>
                            <button onClick={() => handleRemoveSecPin(p)} style={{ border:"none", background:"none", color:RED_DARK, fontWeight:700, cursor:"pointer", fontSize:16, padding:0, lineHeight:1 }} title="Remove PIN">✕</button>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div style={{ background:WHITE, border:`1px solid ${BORDER_GREY}`, borderRadius:10, overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,0.06)" }}>
                <div style={{ background:NAVY, padding:"14px 20px", display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
                  ...
                  <div style={{ fontSize:13, color:"rgba(255,255,255,0.7)" }}>
                    Total bookings: {Object.keys(bookings).length}
                  </div>
                </div>
                            ...
                            {[1,2,3,4].map(sheetNum => {
                              ...
                              return (
                                <div key={sheetNum} style={{ padding:"10px 12px", borderRight:sheetNum<4?`1px solid ${BORDER_GREY}`:"none", background:entry?RED_LIGHT:WHITE }}>
                                  ...
                                  {entry ? (
                                    <>
                                      <div style={{ fontSize:12, color:CHARCOAL, marginBottom:6, wordBreak:"break-all" }}>{entry[1].email}</div>
                                      {entry[1].assignedByAdmin && <div style={{ fontSize:11, color:NAVY, marginBottom:4, fontStyle:"italic" }}>Admin assigned</div>}
                                      <button onClick={() => adminCancel(entry[0])} style={{ width:"100%", padding:"5px 0", border:`1px solid ${RED_BORDER}`, borderRadius:5, background:WHITE, color:RED_DARK, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Cancel</button>
                                    </>
                                  ) : (
                                    <>
                                      <div style={{ fontSize:12, color:"#aaa", marginBottom:6 }}>Available</div>
                                      <button onClick={() => { setAssignModal({ club, slotId:slot.id, slotLabel:slot.label, sheet:sheetNum }); setAssignEmail(""); setAssignError(""); }} style={{ width:"100%", padding:"5px 0", border:`1px solid ${NAVY}`, borderRadius:5, background:NAVY_LIGHT, color:NAVY, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Assign</button>
                                    </>
                                  )}
                                </div>
                              );
          ...
          <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:20 }}>
            <div style={{ background:WHITE, borderRadius:12, padding:"28px 28px 24px", maxWidth:420, width:"100%", boxShadow:"0 8px 40px rgba(0,0,0,0.25)" }}>
              ...
              <input
                ...
                style={{ width:"100%", padding:"11px 13px", borderRadius:7, border:`2px solid ${assignError?RED:BORDER_GREY}`, fontSize:16, fontFamily:"inherit", color:CHARCOAL, boxSizing:"border-box", outline:"none" }}
              />
  ...
  return (
    <div style={{ minHeight:"100vh", background:LIGHT_GREY, fontFamily:"'Arial',sans-serif", color:CHARCOAL }}>
      ...
      {/* Header */}
      <div style={{ background:`linear-gradient(135deg,${RED_DARK} 0%,${RED} 100%)`, boxShadow:"0 3px 12px rgba(0,0,0,0.25)" }}>
        ...
        <div style={{ padding:"20px 28px 22px", display:"flex", alignItems:"center", gap:18, flexWrap:"wrap" }}>
          ...
          <div style={{ marginLeft:"auto", display:"flex", flexDirection:"column", gap:8, alignItems:"flex-end" }}>
            <div style={{ background:"rgba(255,255,255,0.15)", border:"1px solid rgba(255,255,255,0.35)", borderRadius:6, padding:"8px 16px", fontSize:14, color:WHITE, fontWeight:600, whiteSpace:"nowrap" }}>📅 March 25, 2026 Only</div>
            <button onClick={() => setView("admin")} style={{ background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.3)", borderRadius:6, padding:"6px 14px", fontSize:12, color:"rgba(255,255,255,0.75)", cursor:"pointer", fontFamily:"inherit", letterSpacing:"0.04em" }}>⚙️ Admin</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding:"24px 20px", maxWidth:960, margin:"0 auto" }}>
        ...
        <div style={{ background:WHITE, border:`1px solid ${BORDER_GREY}`, borderRadius:10, padding:"20px 22px", marginBottom:18, boxShadow:"0 1px 4px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
            ...
            />
            <button onClick={handleSetEmail} style={{ padding:"13px 28px", borderRadius:7, border:"none", background:enteredEmail===emailInput.trim().toLowerCase()&&enteredEmail?RED_DARK:RED, color:WHITE, fontWeight:700, cursor:"pointer", fontSize:18, fontFamily:"inherit" }}>
          ...
          {loading ? (
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {SLOTS.map(slot => {
                ...
                const slotEntries = getSlotBookings(club, slot.id);
                const bookedCount = slotEntries.length;
                const sheetsLeft = SHEETS_PER_SLOT - bookedCount;
        ...
        <div style={{ display:"flex", gap:18, marginTop:14, fontSize:15, color:MID_GREY, flexWrap:"wrap", alignItems:"center" }}>
          {[[GREEN,"Your booking"],[RED_LIGHT,"Taken"],[ICE_BLUE,"Available"],[AMBER_BG,"Ice cleaning"]].map(([bg,label]) => (
            <span key={label} style={{ display:"flex", alignItems:"center", gap:6 }}><span style={{ width:16, height:16, borderRadius:3, background:bg, border:`1px solid ${BORDER_GREY}`, display:"inline-block" }}></span>{label}</span>
          ))}
